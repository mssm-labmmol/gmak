...
class ParameterGrid:
    ...
    def read_final_properties (self, prop, fn_value, fn_err):
        data_value = np.loadtxt(fn_value, usecols=(0,))
        data_err   = np.loadtxt(fn_err,   usecols=(0,))
        for i,x in enumerate(self.grid_points):
            if (prop == 'density'):
                x.estimated_properties[prop] = Density (data_value[i], data_err[i])
            elif (prop == 'gamma'):
                x.estimated_properties[prop] = Gamma (data_value[i], data_err[i])
            elif (prop == 'dhvap'):
                # this is not 100% accurate because I am not separating
                # the contributions of polarization and other corrections
                # I am putting everything in the gas value
                x.estimated_properties[prop] = dHvap (0.0, data_value[i], 0, 0, data_err[i], 0, 0, 1, 0)
   ...
   #         corrs, nmols, temperature):
    ...
    def plot_property_diff_to_file (self, prop, ref, filename):
        ...
        # or by the natural limits
        #bigvalue     = np.max(np.abs(data))
        #cbox_limits  = (-bigvalue,+bigvalue)
...
if __name__ == "__main__":
    ...
    for nsteps in range(optimizer.maxSteps):
        ...
        for prop in properties:
            ...
            grid.read_final_properties(prop, thisRunOutputs + "/" + prop + "_EA_k.dat",\
                    thisRunOutputs + "/" + prop + "_dEA_k.dat")
            #
